...
response_set = JsonAnswer.objects.filter(active=2).exclude(interface = 2)
community_organizer_set = UserRoles.objects.filter(user__is_active=True)
...
user_set = User.objects.filter(is_active=1)
boundary_set = Boundary.objects.filter(active=2)

class DashBoardSnapShot():
    @staticmethod
    def get_partner_users(partner):
        partner_users = community_organizer_set.filter(partner=partner,
        role_type__slug='data-entry-operator')
        ...
        for pc,partner in enumerate(partner_set):
            ...
            for c,cd in enumerate(calculation_days):
                ...
                for community_organizer in  partner_users:
                    responses = response_set.filter(user=community_organizer,created__gte=cd.get('start_date'),created__lte=cd.get('end_date'))
                    last_collected = None
                    try:
    ...
    def get_village_count(user_responses,partner):
        ...
        partner_villages_count = [i[0]['boundary']['id'] for i in list(user_responses.filter(survey__id__in=village_survey).distinct('cluster').values_list('cluster',flat=True))]
    ...
    def get_beneficiary_count(user_responses,partner):
        ...
        for bt in beneficiary_types:
            ...
            beneficiers = []
            for i in list(user_responses.filter(survey_id__in=beneficiary_survey).distinct('cluster').values_list('cluster',flat=True)):
                try:
            ...
            beneficary_count.append({bt.name:{'village_count':village_count,'others_count':others_count,
                ...
                'beneficiary_reached':len(beneficiers),'partner_villages_count':len(partner_villages)}})
