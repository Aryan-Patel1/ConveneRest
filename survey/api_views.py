...
def applogin(request,**kwargs):
    ...
    response.update({'responseType' : response_type, 'updates':5,'master_data_file_url':HOST_URL+'/static/partner_masterdata/'+str(get_user_partner(userrole_obj))+'_masterdata.txt'})
...
def create_answers(user, **ans_params):
    ...
    if str(ben_uuid) != '0':
        beneficiary = Beneficiary.objects.get(uuid=ben_uuid).id
    else:
        beneficiary = 0
    if str(fac_uuid) != '0':
