    ...
    def handle(*args, **kwargs):
        ...
        print "users ",users
            for survey in survey_list:
                survey_workflow = WorkFlowSurveyRelation.objects.get_or_none(
                    survey=survey)
        print "workflow ",survey_workflow
                if survey_workflow:
                    json_responses = JsonAnswer.objects.filter(active=2,
                        survey=survey, user__in=users)
                    tagged_responses = WorkFlowBatch.objects.filter(
                        partner=partner, current_status=survey_workflow).values_list('response_ids', flat=True)
                    tagged_ids = list(
                        set(itertools.chain.from_iterable(tagged_responses)))
                    new_keys = json_responses.exclude(
                        id__in=tagged_ids).values_list('id', flat=True)
                    # store these keys in response batch tag table
                    if new_keys:
                        wfb_name = str(partner.id) + '_' + str(survey.id) + '_' + str(
                            survey_workflow.id) + '_' + time.strftime('%y_%m_%d_%h_%s')
